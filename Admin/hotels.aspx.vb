...
Partial Class Admin_hotels
        ...
        Try
            ...
            If FileUpload.HasFile Then
                ...
                If FileUpload.PostedFile.ContentType.Split("/")(0) = "image" Then
                    ...
                    FileUpload.SaveAs(Server.MapPath("../images/hotels/actual/" + File))
                    ResizeImageAndSave(800, 800, Context.Server.MapPath("../images/hotels/actual/" & File), "../images/hotels/" + File)
                End If
            End If
            ...
            Dim Result As String = BLL.AddUpdateHotels(hfId.Value, txtDestination.Text, ddlHotelType.SelectedValue, txtProductDesc.Text, ddlPlace.SelectedValue, ddlState.SelectedValue, File, user, ddlStatus.SelectedValue)
            showdata()
            div1.Visible = False
            div2.Visible = True
    ...
    Protected Sub DataDisplay_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles DataDisplay.RowCommand
        If e.CommandName = "Delete1" Then
            BLL.ExecNonQuery("Update tblHotels Set Deactivated=2 Where Cnt=@Cnt and UserName=@UserName", "@Cnt", e.CommandArgument, "@UserName", System.Configuration.ConfigurationManager.AppSettings("UserName"))
            showdata()
        ElseIf e.CommandName = "Edit1" Then
            hfId.Value = e.CommandArgument
            Dim dr As SqlDataReader = BLL.ExecDataReader("select * from tblHotels where Cnt=@Cnt and UserName=@UserName", "@Cnt", e.CommandArgument, "@UserName", System.Configuration.ConfigurationManager.AppSettings("UserName"))
            If dr.Read Then
                txtDestination.Text = dr("HotelName").ToString()
                ddlHotelType.SelectedValue = dr("HotelType").ToString()
    ...
    End Sub
    Sub showdata()
        DataDisplay.DataSource = BLL.ExecDataTable("Select *,(Case when DeActivated=0 then 'Active' Else 'DeActive' End)Status from tblHotels Where Deactivated<=1 and UserName=@UserName", "@UserName", System.Configuration.ConfigurationManager.AppSettings("UserName"))
