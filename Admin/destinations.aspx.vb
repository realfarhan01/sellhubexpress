...
Partial Class Admin_destinations
        ...
        Try
            ...
            If FileUpload.HasFile Then
                ...
                If FileUpload.PostedFile.ContentType.Split("/")(0) = "image" Then
                    ...
                    FileUpload.SaveAs(Server.MapPath("../images/destinations/actual/" + File))
                    ResizeImageAndSave(800, 800, Context.Server.MapPath("../images/destinations/actual/" & File), "../images/destinations/" + File)
                End If
            End If
            Dim user As String = System.Configuration.ConfigurationManager.AppSettings("UserName")
            Dim pageurl As String = BLL.GetPageURL(txtDestination.Text)
            'Dim FolderPath As String = System.Configuration.ConfigurationManager.AppSettings("UserName")
            Dim Result As String = BLL.AddUpdateDestinations(hfId.Value, txtDestination.Text, txtShortDesc.Text, txtProductDesc.Text, ddlPlace.SelectedValue, ddlState.SelectedValue, txtLocationEmbed.Text, File, user, ddlStatus.SelectedValue, pageurl)
            showdata()
            div1.Visible = False
            div2.Visible = True
    ...
    Protected Sub DataDisplay_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles DataDisplay.RowCommand
        If e.CommandName = "Delete1" Then
            BLL.ExecNonQuery("Update tblDestinations Set Deactivated=2 Where Cnt=@Cnt and UserName=@UserName", "@Cnt", e.CommandArgument, "@UserName", System.Configuration.ConfigurationManager.AppSettings("UserName"))
            showdata()
        ElseIf e.CommandName = "Edit1" Then
            hfId.Value = e.CommandArgument
            Dim dr As SqlDataReader = BLL.ExecDataReader("select * from tblDestinations where Cnt=@Cnt and UserName=@UserName", "@Cnt", e.CommandArgument, "@UserName", System.Configuration.ConfigurationManager.AppSettings("UserName"))
            If dr.Read Then
                ...
                BindPlace(dr("State"))
                ddlPlace.SelectedValue = dr("Place")
    ...
    End Sub
    Sub showdata()
        DataDisplay.DataSource = BLL.ExecDataTable("Select *,(Case when DeActivated=0 then 'Active' Else 'DeActive' End)Status from tblDestinations Where Deactivated<=1 and UserName=@UserName", "@UserName", System.Configuration.ConfigurationManager.AppSettings("UserName"))
